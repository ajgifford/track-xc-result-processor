 ...
 *
 * Generates optimized meet entries based on team's current rankings.
 */

import * as fs from 'fs';
import * as path from 'path';
import * as readline from 'readline';
import { SimpleTeamEntryOptimizer } from './team-entry-optimizer-simple';
...
async function main() {
    console.log('═══════════════════════════════════════════════════════════');
    console.log('  Team Entry Optimizer - SIMPLE METHOD');
    console.log('═══════════════════════════════════════════════════════════\n');
    console.log('This optimizer uses your team\'s current rankings to');
    console.log('recommend optimal event entries (assumes rank = placement).\n');
    ...
    try {
        ...
        if (scopeAnswer.trim() === '1') {
            ...
            const result = SimpleTeamEntryOptimizer.optimize(
                rankingsDir,
                season,
                teamName,
                gender,
        ...
        for (const { result, outputPrefix } of results) {
            const jsonPath = path.join(outputDir, `${outputPrefix}_simple.json`);
            const txtPath = path.join(outputDir, `${outputPrefix}_simple.txt`);

            SimpleTeamEntryOptimizer.saveJSON(result, jsonPath);
            SimpleTeamEntryOptimizer.saveReport(result, txtPath);
